...
Inherits from HuggingFace PretrainedConfig for full HF ecosystem compatibility.
"""
...
class HelixConfig(PretrainedConfig):
    """
    Configuration class for HelixLM.
    ...
    Inherits from ``transformers.PretrainedConfig`` so HelixLM models can be
    saved / loaded with standard HF utilities (``save_pretrained``,
    ``from_pretrained``).
    """
    ...
    bos_token_id: Optional[int] = None  # BOS token ID
    ...
    # Generation defaults
    # ------------------------------------------------------------------
    max_new_tokens: int = 50           # Default generation length
    ...
    def __init__(self, **kwargs):
        ...
        config_kwargs = {k: v for k, v in kwargs.items() if k in all_fields}
        # Pass vocab_size and other HF-standard args to PretrainedConfig
        super().__init__(
            vocab_size=config_kwargs.get("vocab_size", self.vocab_size),
            pad_token_id=config_kwargs.get("pad_token_id", self.pad_token_id),
            eos_token_id=config_kwargs.get("eos_token_id", self.eos_token_id),
            bos_token_id=config_kwargs.get("bos_token_id", self.bos_token_id),
        )
        # Set all HelixConfig fields
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        d = asdict(self)
        # Include PretrainedConfig fields
        base = super().to_dict()
        base.update(d)
        return base
